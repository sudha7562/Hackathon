...
exports.updateManifest = exports.getManifest = exports.validateManifest = exports.buildTeamsAppPackage = exports.publishTeamsApp = exports.updateTeamsApp = exports.createTeamsApp = void 0;
...
const utils_2 = require("../../../plugins/resource/appstudio/utils/utils");
...
async function publishTeamsApp(ctx, inputs, envInfo, tokenProvider, telemetryProps) {
    ...
    if (inputs.appPackagePath) {
        ...
        const buildPackage = await buildTeamsAppPackage(inputs.projectPath, envInfo, false, telemetryProps);
        if (buildPackage.isErr()) {
            return teamsfx_api_1.err(buildPackage.error);
        }
        archivedFile = await fs_extra_1.default.readFile(buildPackage.value);
 ...
 */
async function buildTeamsAppPackage(projectPath, envInfo, withEmptyCapabilities = false, telemetryProps) {
...
async function getManifest(projectPath, envInfo, telemetryProps) {
    var _a, _b, _c, _d, _e, _f;
    // Read template
    const manifestTemplateRes = await utils_1.readAppManifest(projectPath);
    ...
    const customizedKeys = utils_2.getCustomizedKeys("", JSON.parse(manifestString));
    if (telemetryProps) {
    ...
    if (isLocalDebug || ((_a = manifest.validDomains) === null || _a === void 0 ? void 0 : _a.length) === 0) {
        const validDomains = [];
        const tabEndpoint = (_b = envInfo.state[constants_3.ComponentNames.TeamsTab]) === null || _b === void 0 ? void 0 : _b.endpoint;
        const tabDomain = (_c = envInfo.state[constants_3.ComponentNames.TeamsTab]) === null || _c === void 0 ? void 0 : _c.domain;
        if (tabDomain) {
        ...
        const botId = (_d = envInfo.state[constants_3.ComponentNames.TeamsBot]) === null || _d === void 0 ? void 0 : _d.botId;
        const botDomain = (_e = envInfo.state[constants_3.ComponentNames.TeamsBot]) === null || _e === void 0 ? void 0 : _e.validDomain;
        if (botId) {
            if (!botDomain) {
                return teamsfx_api_1.err(results_1.AppStudioResultFactory.UserError(errors_1.AppStudioError.GetRemoteConfigFailedError.name, errors_1.AppStudioError.GetRemoteConfigFailedError.message(localizeUtils_1.getLocalizedString("plugins.appstudio.dataRequired", "validDomain"), isProvisionSucceeded), constants_1.HelpLinks.WhyNeedProvision));
            }
            else {
        ...
        for (const domain of validDomains) {
            if (((_f = manifest.validDomains) === null || _f === void 0 ? void 0 : _f.indexOf(domain)) == -1) {
...
async function updateManifest(ctx, inputs) {
    ...
    const manifestFileName = await utils_1.getTeamsAppManifestPath(inputs.projectPath);
    if (!(await fs_extra_1.default.pathExists(manifestFileName))) {
        const isProvisionSucceeded = ctx.envInfo.state["solution"].provisionSucceeded;
