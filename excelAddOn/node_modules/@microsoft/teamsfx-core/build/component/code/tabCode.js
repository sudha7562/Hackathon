...
const utils_3 = require("../../plugins/resource/frontend/utils");
...
let TabCodeProvider = class TabCodeProvider {
    ...
    async generate(ctx, inputs, actionContext) {
        ...
        inputs.folder =
            inputs.folder ||
                (inputs[question_1.CoreQuestionNames.ProgrammingLanguage] === "csharp"
                    ? ""
    ...
    async doBlazorBuild(tabPath) {
        ...
        try {
            await utils_3.Utils.execute(command, tabPath);
        }
        catch (e) {
    ...
    async doReactBuild(tabPath, envName, telemetryReporter) {
        ...
        await utils_3.Utils.execute("install:teamsfx" in scripts
            ? constants_1.Commands.InstallNodePackages
            : constants_1.Commands.DefaultInstallNodePackages, tabPath);
        if ("build:teamsfx" in scripts) {
            await utils_3.Utils.execute(constants_1.Commands.BuildFrontend, tabPath, {
                TEAMS_FX_ENV: envName,
            });
        }
        else {
            const envs = await env_1.loadEnvFile(env_1.envFilePath(envName, tabPath));
            await utils_3.Utils.execute(constants_1.Commands.DefaultBuildFrontend, tabPath, Object.assign(Object.assign({}, envs.customizedRemoteEnvs), envs.teamsfxRemoteEnvs));
        }
...
tslib_1.__decorate([
    lib_1.hooks([
        actionExecutionMW_1.ActionExecutionMW({
            errorSource: constants_1.FrontendPluginInfo.ShortName,
