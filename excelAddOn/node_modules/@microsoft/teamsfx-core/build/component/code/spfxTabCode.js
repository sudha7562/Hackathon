...
const generatorChecker_1 = require("../../plugins/resource/spfx/depsChecker/generatorChecker");
const yoChecker_1 = require("../../plugins/resource/spfx/depsChecker/yoChecker");
const error_1 = require("../../plugins/resource/spfx/error");
const constants_2 = require("../../plugins/resource/spfx/utils/constants");
const progress_helper_1 = require("../../plugins/resource/spfx/utils/progress-helper");
const questions_1 = require("../../plugins/resource/spfx/utils/questions");
const utils_1 = require("../../plugins/resource/spfx/utils/utils");
const utils4v2_1 = require("../../plugins/resource/utils4v2");
const cpUtils_1 = require("../../plugins/solution/fx-solution/utils/depsChecker/cpUtils");
...
async function scaffoldSPFx(context, inputs, outputFolderPath) {
    ...
    const ui = context.userInteraction || context.ui;
    ...
    try {
        ...
        let framework, solutionName = undefined;
        const isAddSpfx = inputs[questions_1.SPFXQuestionNames.framework_type] === undefined && common_1.isSPFxMultiTabEnabled();
        if (!isAddSpfx) {
            framework = inputs[questions_1.SPFXQuestionNames.framework_type];
            solutionName =
                ((_a = context.projectSetting) === null || _a === void 0 ? void 0 : _a.appName) ||
                    ((_b = context.projectSettings) === null || _b === void 0 ? void 0 : _b.appName);
        }
        ...
        yoEnv.PATH = tools_1.isYoCheckerEnabled()
            ? `${await (await yoChecker.getBinFolders()).join(path.delimiter)}${path.delimiter}${(_c = process.env.PATH) !== null && _c !== void 0 ? _c : ""}`
            : process.env.PATH;
        const args = [
        ...
        if (isAddSpfx) {
            ...
            const addCapRes = await manifestProvider.addCapabilities(context.manifestProvider
                ? context
                : utils4v2_1.convert2Context(context, true).context, inputs, capabilitiesToAddManifest);
            if (addCapRes.isErr())
            ...
            for (const capability of capabilitiesToAddManifest) {
                const addCapRes = await manifestProvider.updateCapability(context.manifestProvider
                    ? context
                    : utils4v2_1.convert2Context(context, true).context, inputs, capability);
                if (addCapRes.isErr())
                    return teamsfx_api_1.err(addCapRes.error);
            }
        }
        ...
        if (error.name === "DependencyInstallFailed") {
            ...
            (_d = context.logProvider) === null || _d === void 0 ? void 0 : _d.error(`We've encountered some issues when trying to install prerequisites under HOME/.fx folder.  Learn how to remediate by going to this link(aka.ms/teamsfx-spfx-help) and following the steps applicable to your system: \n ${yoMessage} \n ${generatorMessage}`);
            (_e = context.logProvider) === null || _e === void 0 ? void 0 : _e.error(`Teams Toolkit recommends using ${yoInfo.displayName} ${genInfo.displayName}`);
        }
        if (error.message &&
            error.message.includes("'yo' is not recognized as an internal or external command")) {
            (_f = context.logProvider) === null || _f === void 0 ? void 0 : _f.error("NPM v6.x with Node.js v12.13.0+ (Erbium) or Node.js v14.15.0+ (Fermium) is recommended for spfx scaffolding and later development. You can use correct version and try again.");
        }
