...
async function getQuestionsForInit(inputs) {
    ...
    if (inputs.platform !== teamsfx_api_1.Platform.CLI) {
        node.addChild(new teamsfx_api_1.QTreeNode(question_2.QuestionRootFolder));
    }
    node.addChild(new teamsfx_api_1.QTreeNode(question_2.createAppNameQuestion(false)));
    const solution = typedi_1.Container.get(constants_1.BuiltInSolutionNames.azure);
...
async function getQuestionsForCreateProjectV3(inputs) {
    ...
    const spfxPlugin = typedi_1.Container.get(constants_1.BuiltInFeaturePluginNames.spfx);
    const spfxRes = await spfxPlugin.getQuestionsForAddInstance(context, inputs);
    if (spfxRes.isOk()) {
        if ((_a = spfxRes.value) === null || _a === void 0 ? void 0 : _a.data) {
            spfxRes.value.condition = { contains: question_1.TabSPFxItem.id };
