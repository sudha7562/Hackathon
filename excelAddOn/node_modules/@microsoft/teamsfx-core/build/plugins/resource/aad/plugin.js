...
class AadAppForTeamsImpl {
    ...
    async postProvision(ctx, isLocalDebug = false) {
        ...
        const isVs = common_1.isVSProject(ctx.projectSettings);
    ...
    async updatePermission(ctx) {
        var _a, _b, _c, _d, _e;
        telemetry_1.TelemetryUtils.init(ctx);
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.StartUpdatePermission);
        ...
        if (skip) {
            ...
            configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.EndUpdatePermission);
            return results_1.ResultFactory.Success();
        }
        ...
        for (const config of configs) {
            await aadAppClient_1.AadAppClient.updateAadAppPermission(constants_1.Messages.EndUpdatePermission.telemetry, config.objectId, permissions);
        }
    ...
    async generateArmTemplates(ctx) {
        var _a;
        telemetry_1.TelemetryUtils.init(ctx);
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.StartGenerateArmTemplates);
        ...
        if (capabilities.includes(question_1.TabOptionItem.id) ||
            capabilities.includes(question_1.BotOptionItem.id) ||
        ...
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.EndGenerateArmTemplates);
        return results_1.ResultFactory.Success(result);
    }
    async checkPermission(ctx, userInfo) {
        telemetry_1.TelemetryUtils.init(ctx);
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.StartCheckPermission);
        await tokenProvider_1.TokenProvider.init({ m365: ctx.m365TokenProvider }, tokenProvider_1.TokenAudience.Graph);
        const config = new configs_1.CheckGrantPermissionConfig();
        await config.restoreConfigFromContext(ctx);
        const userObjectId = userInfo.aadId;
        const isAadOwner = await aadAppClient_1.AadAppClient.checkPermission(constants_1.Messages.EndCheckPermission.telemetry, config.objectId, userObjectId);
        const result = [
            {
        ...
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.EndCheckPermission);
        return results_1.ResultFactory.Success(result);
    }
    async listCollaborator(ctx) {
        telemetry_1.TelemetryUtils.init(ctx);
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.StartListCollaborator);
        await tokenProvider_1.TokenProvider.init({ m365: ctx.m365TokenProvider }, tokenProvider_1.TokenAudience.Graph);
        const objectId = configs_1.ConfigUtils.getAadConfig(ctx, constants_1.ConfigKeys.objectId, false);
        if (!objectId) {
            throw results_1.ResultFactory.SystemError(errors_1.GetConfigError.name, errors_1.ConfigErrorMessages.GetConfigError(constants_1.ConfigKeys.objectId, constants_1.Plugins.pluginName));
        }
        ...
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.EndListCollaborator);
        return results_1.ResultFactory.Success(owners);
    }
    async grantPermission(ctx, userInfo) {
        telemetry_1.TelemetryUtils.init(ctx);
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.StartGrantPermission);
        await tokenProvider_1.TokenProvider.init({ m365: ctx.m365TokenProvider }, tokenProvider_1.TokenAudience.Graph);
        const config = new configs_1.CheckGrantPermissionConfig(true);
        await config.restoreConfigFromContext(ctx);
        const userObjectId = userInfo.aadId;
        await aadAppClient_1.AadAppClient.grantPermission(ctx, config.objectId, userObjectId);
        const result = [
            {
                name: constants_1.Constants.permissions.name,
                type: constants_1.Constants.permissions.type,
        ...
        configs_1.Utils.addLogAndTelemetry(ctx.logProvider, constants_1.Messages.EndGrantPermission);
        return results_1.ResultFactory.Success(result);
    }
    ...
    static async getUpdatePermissionConfigs(ctx) {
        ...
        if (ctx.config.get(configs_1.Utils.addLocalDebugPrefix(true, constants_1.ConfigKeys.objectId))) {
            localAad = true;
        }
    ...
    async loadAndBuildManifest(ctx) {
        ...
        if (ctx.envInfo.envName === "local") {
            isProvisionSucceeded = !!((_a = ctx.envInfo.state.get(solution_1.PluginNames.AAD)) === null || _a === void 0 ? void 0 : _a.get(solution_1.REMOTE_AAD_ID));
        }
        else {
            isProvisionSucceeded = !!((_b = ctx.envInfo.state
                .get("solution")) === null || _b === void 0 ? void 0 : _b.get(solution_1.SOLUTION_PROVISION_SUCCEEDED));
        }
    ...
    async writeManifestFileToBuildFolder(manifest, ctx) {
