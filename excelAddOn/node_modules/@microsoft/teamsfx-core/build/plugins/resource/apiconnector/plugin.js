...
class ApiConnectorImpl {
    async scaffold(ctx, inputs) {
        ...
        // CLI checker
        const bot = core_1.isV3()
            ? projectSettingsHelperV3_1.hasBot(ctx.projectSetting)
            : (_b = (_a = ctx.projectSetting.solutionSettings) === null || _a === void 0 ? void 0 : _a.activeResourcePlugins) === null || _b === void 0 ? void 0 : _b.includes(constants_2.ResourcePlugins.Bot);
        const hasFunc = core_1.isV3()
            ? projectSettingsHelperV3_1.hasApi(ctx.projectSetting)
            : (_d = (_c = ctx.projectSetting.solutionSettings) === null || _c === void 0 ? void 0 : _c.activeResourcePlugins) === null || _d === void 0 ? void 0 : _d.includes(constants_2.ResourcePlugins.Function);
        ...
        try {
            ...
            const logMessage = localizeUtils_1.getLocalizedString("plugins.apiConnector.Log.CommandSuccess", filesChanged.reduce((previousValue, currentValue) => previousValue +
                ...
                "\n", ""));
            (_e = ctx.logProvider) === null || _e === void 0 ? void 0 : _e.info(logMessage); // Print generated/updated files for users
            if (inputs.platform != teamsfx_api_1.Platform.CLI) {
                (_f = ctx.userInteraction) === null || _f === void 0 ? void 0 : _f.showMessage("info", msg, false, "OK", utils_1.Notification.READ_MORE).then((result) => {
                    var _a;
        ...
        const result = config.ComponentType.map((item) => {
            return path.join(projectPath, item, utils_1.getSampleFileName(config.APIName, languageType));
        });
    ...
    buildAADAuthQuestion(ctx, inputs) {
        let aad;
        if (core_1.isV3()) {
            aad = projectSettingsHelperV3_1.hasAAD(ctx.projectSetting);
        }
        else {
            const solutionSettings = utils_2.getAzureSolutionSettings(ctx);
            aad = common_1.isAADEnabled(solutionSettings);
        }
        let node;
