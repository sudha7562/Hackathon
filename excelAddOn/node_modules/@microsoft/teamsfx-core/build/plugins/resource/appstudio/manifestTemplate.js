        ...
        case "staticTab":
            exceed =
                manifest.value.staticTabs !== undefined &&
                    manifest.value.staticTabs.length >= constants_1.STATIC_TABS_MAX_ITEMS;
            return teamsfx_api_1.ok(exceed);
...
async function addCapabilities(projectRoot, capabilities, inputs) {
    ...
    let staticTabIndex = (_b = (_a = manifest.staticTabs) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0 ? _b : 0;
        ...
        switch (capability.name) {
            case "staticTab":
                ...
                if (capability.snippet) {
                    ...
                    if (capability.existingApp) {
                        const template = tools_1.deepCopy(constants_1.STATIC_TABS_TPL_EXISTING_APP[0]);
                        template.entityId = "index" + staticTabIndex;
                        manifest.staticTabs.push(template);
                    }
                    else {
                        const template = tools_1.deepCopy(constants_1.STATIC_TABS_TPL_FOR_MULTI_ENV[0]);
                        template.entityId = "index" + staticTabIndex;
                        manifest.staticTabs.push(template);
                        if (((_a = manifest.validDomains) === null || _a === void 0 ? void 0 : _a.indexOf("{{state.fx-resource-frontend-hosting.domain}}")) == -1) {
                            (_b = manifest.validDomains) === null || _b === void 0 ? void 0 : _b.push("{{state.fx-resource-frontend-hosting.domain}}");
                        }
                    }
                    staticTabIndex++;
                }
            ...
            case "configurableTab":
                ...
                if (capability.snippet) {
                    ...
                    if (capability.existingApp) {
                        manifest.configurableTabs = manifest.configurableTabs.concat(constants_1.CONFIGURABLE_TABS_TPL_EXISTING_APP);
                    }
                    else {
                        manifest.configurableTabs = manifest.configurableTabs.concat(constants_1.CONFIGURABLE_TABS_TPL_FOR_MULTI_ENV);
                        if (((_c = manifest.validDomains) === null || _c === void 0 ? void 0 : _c.indexOf("{{state.fx-resource-frontend-hosting.domain}}")) == -1) {
                            (_d = manifest.validDomains) === null || _d === void 0 ? void 0 : _d.push("{{state.fx-resource-frontend-hosting.domain}}");
                        }
                    }
                }
                break;
            case "Bot":
                ...
                if (capability.snippet) {
                    ...
                    if (capability.existingApp) {
                        manifest.bots = manifest.bots.concat(constants_1.BOTS_TPL_EXISTING_APP);
                    }
                    else {
                        ...
                        if (featureFlags_1.isBotNotificationEnabled()) {
                            ...
                            if (scenarios.includes(question_1.BotScenario.CommandAndResponseBot)) {
                                ...
                                manifest.bots = manifest.bots.concat(constants_1.BOTS_TPL_FOR_COMMAND_AND_RESPONSE);
                            }
                            else if (scenarios.includes(question_1.BotScenario.NotificationBot)) {
                                // notification
                                manifest.bots = manifest.bots.concat(constants_1.BOTS_TPL_FOR_NOTIFICATION);
                            }
                            else {
                                // legacy bot
                                manifest.bots = manifest.bots.concat(constants_1.BOTS_TPL_FOR_MULTI_ENV);
                            }
                        }
                        else {
                            manifest.bots = manifest.bots.concat(constants_1.BOTS_TPL_FOR_MULTI_ENV);
                        }
                        (_e = manifest.validDomains) === null || _e === void 0 ? void 0 : _e.push("{{state.fx-resource-bot.validDomain}}");
                    }
                }
                break;
            case "MessageExtension":
                ...
                if (capability.snippet) {
                    ...
                    if (capability.existingApp) {
                        manifest.composeExtensions = manifest.composeExtensions.concat(constants_1.COMPOSE_EXTENSIONS_TPL_EXISTING_APP);
                    }
                    else {
                        manifest.composeExtensions = manifest.composeExtensions.concat(constants_1.COMPOSE_EXTENSIONS_TPL_FOR_MULTI_ENV);
                    }
                }
                break;
            case "WebApplicationInfo":
                if (capability.snippet) {
                    ...
                    manifest.webApplicationInfo = constants_1.WEB_APPLICATION_INFO_MULTI_ENV;
                }
                break;
        }
    });
