...
exports.askForProvisionConsentNew = exports.provisionResource = void 0;
...
const provision_1 = require("../v3/provision");
...
function getSubscriptionId(state) {
    if (state && state[constants_1.GLOBAL_CONFIG] && state[constants_1.GLOBAL_CONFIG][constants_1.SUBSCRIPTION_ID]) {
        return state[constants_1.GLOBAL_CONFIG][constants_1.SUBSCRIPTION_ID];
    }
    return "";
}
function getTeamsAppTenantId(state) {
    if (state && state[constants_1.GLOBAL_CONFIG] && state[constants_1.GLOBAL_CONFIG][constants_1.REMOTE_TEAMS_APP_TENANT_ID]) {
        return state[constants_1.GLOBAL_CONFIG][constants_1.REMOTE_TEAMS_APP_TENANT_ID];
    }
    return "";
}
async function provisionResource(ctx, inputs, envInfo, tokenProvider) {
    ...
    ctx.telemetryReporter.sendTelemetryEvent(constants_1.SolutionTelemetryEvent.ProvisionStart, {
        ...
        [constants_1.SolutionTelemetryProperty.SubscriptionId]: getSubscriptionId(envInfo.state),
...
async function provisionResourceImpl(ctx, inputs, envInfo, tokenProvider) {
    ...
    const tenantIdInTokenRes = await provision_1.getM365TenantId(tokenProvider.m365TokenProvider);
    ...
    if (utils_1.isAzureProject(azureSolutionSettings) && projectSettingsHelper_1.hasAzureResource(ctx.projectSetting, true)) {
        ...
        const solutionConfigRes = await provision_1.fillInAzureConfigs(ctx, inputsNew, envInfo, tokenProvider);
        if (solutionConfigRes.isErr()) {
            return teamsfx_api_1.err(solutionConfigRes.error);
        }
        const consentResult = await askForProvisionConsentNew(ctx, tokenProvider.azureAccountProvider, envInfo, hasSwitchedM365Tenant, solutionConfigRes.value.hasSwitchedSubscription, tenantIdInTokenRes.value.tenantUserName, true, tenantIdInConfig, subscriptionIdInState);
        ...
        const consentResult = await askForProvisionConsentNew(ctx, tokenProvider.azureAccountProvider, envInfo, hasSwitchedM365Tenant, false, tenantIdInTokenRes.value.tenantUserName, false, tenantIdInConfig);
        if (consentResult.isErr()) {
            return teamsfx_api_1.err(consentResult.error);
        }
...
async function askForProvisionConsentNew(ctx, azureAccountProvider, envInfo, hasSwitchedM365Tenant, hasSwitchedSubscription, m365AccountName, hasAzureResource, previousM365TenantId, previousSubscriptionId) {
    var _a, _b, _c, _d, _e;
    ...
    if (hasSwitchedM365Tenant && hasSwitchedSubscription) {
        ...
        const botResource = (_d = envInfo.state[constants_4.BuiltInFeaturePluginNames.bot]) !== null && _d !== void 0 ? _d : envInfo.state[constants_5.ComponentNames.TeamsBot];
    ...
    do {
        ...
        (_e = ctx.telemetryReporter) === null || _e === void 0 ? void 0 : _e.sendTelemetryEvent(telemetry_1.TelemetryEvent.ConfirmProvision, envInfo.envName
            ? {
                ...
                [constants_1.SolutionTelemetryProperty.SubscriptionId]: getSubscriptionId(envInfo.state),
                [constants_1.SolutionTelemetryProperty.M365TenantId]: getTeamsAppTenantId(envInfo.state),
                [constants_1.SolutionTelemetryProperty.PreviousM365TenantId]: previousM365TenantId,
                [constants_1.SolutionTelemetryProperty.PreviousSubsriptionId]: previousSubscriptionId !== null && previousSubscriptionId !== void 0 ? previousSubscriptionId : "",
                [constants_1.SolutionTelemetryProperty.ConfirmRes]: !confirm
                    ? "Error"
        ...
        if (confirm !== provisionText) {
            if (confirm === learnMoreText) {
                ...
                return teamsfx_api_1.err(new teamsfx_api_1.UserError(constants_1.SolutionSource, "CancelProvision", "CancelProvision"));
            }
        }
    } while (confirm === learnMoreText);
    return teamsfx_api_1.ok(teamsfx_api_1.Void);
}
exports.askForProvisionConsentNew = askForProvisionConsentNew;
