...
async function backupFiles(env, projectPath) {
    const time = formatDate();
    const backupFolder = await getBackupFolder(projectPath);
    // state file
    const stateFileBackupRes = await backupFile(projectPath, env, stateFileNameTemplate, stateFolder, backupFolder, time, jsonSuffix);
    if (stateFileBackupRes.isErr()) {
        return teamsfx_api_1.err(stateFileBackupRes.error);
    }
    // user data file
    const userDataFileBackupRes = await backupFile(projectPath, env, userDateFileNameTemplate, stateFolder, backupFolder, time, userDataSuffix);
    if (userDataFileBackupRes.isErr()) {
        return teamsfx_api_1.err(userDataFileBackupRes.error);
    }
    // Azure parameter file
    if (env !== "local") {
        const azureParameterFileBackupRes = await backupFile(projectPath, env, azureParameterFileNameTemplate, configFolder, backupFolder, time, jsonSuffix);
    ...
    if (await fs_extra_1.default.pathExists(backupFolder)) {
        await projectMigrator_1.addPathToGitignore(projectPath, backupFolder, core_1.TOOLS.logProvider);
    }
    return teamsfx_api_1.ok(undefined);
}
exports.backupFiles = backupFiles;
async function backupFile(projectPath, env, fileNameTemplate, folder, backupFolder, time, suffix) {
    const sourceFileName = fileNameTemplate.replace(teamsfx_api_1.EnvNamePlaceholder, env);
    const sourceFile = path_1.default.join(path_1.default.join(projectPath, folder), sourceFileName);
    try {
        const backupFileFolder = path_1.default.join(backupFolder, folder);
        const backupFileName = generateBackupFileName(sourceFileName, backupFileFolder, suffix, time);
        const backupFile = path_1.default.join(path_1.default.join(backupFolder, folder), backupFileName);
        await copyFileToBackupFolderIfExists(sourceFile, backupFile, path_1.default.join(backupFolder, folder));
        return teamsfx_api_1.ok(undefined);
    }
    catch (exception) {
        const error = new teamsfx_api_1.UserError(constants_1.SolutionSource, constants_1.SolutionError.FailedToBackupFiles, localizeUtils_1.getDefaultString("core.backupFiles.FailedToBackupFiles", sourceFile), localizeUtils_1.getLocalizedString("core.backupFiles.FailedToBackupFiles", sourceFile));
...
async function generateReport(backupFolder) {
    try {
