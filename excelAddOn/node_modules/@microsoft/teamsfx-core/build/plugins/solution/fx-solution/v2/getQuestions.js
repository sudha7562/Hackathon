...
const constants_5 = require("../../../resource/aad/constants");
...
async function getQuestions(ctx, inputs, envInfo, tokenProvider) {
    ...
    if (stage === teamsfx_api_1.Stage.provision) {
        ...
        if (inputs.platform === teamsfx_api_1.Platform.VSCode && inputs[constants_5.Constants.INCLUDE_AAD_MANIFEST] === "yes") {
            return teamsfx_api_1.ok(node);
        }
        if (isDynamicQuestion) {
        ...
        if (plugins.length === 0 && inputs[constants_5.Constants.INCLUDE_AAD_MANIFEST] !== "yes") {
...
async function getQuestionsForAddCapability(ctx, inputs, func, envInfo, tokenProvider) {
    ...
    if (!isDynamicQuestion) {
        ...
        addCapQuestion.staticOptions = [
            ...
            ...(featureFlags_1.isBotNotificationEnabled() ? [question_1.NotificationOptionItem, question_1.CommandAndResponseOptionItem] : []),
    ...
    const appStudioPlugin = typedi_1.default.get(constants_4.BuiltInFeaturePluginNames.appStudio);
...
async function getQuestionsForAddFeature(ctx, inputs, func, envInfo, tokenProvider) {
    ...
    if (inputs.platform === teamsfx_api_1.Platform.CLI_HELP || isApiConnectionAddable) {
        pluginsWithResources.push([ResourcePluginContainer_1.ResourcePluginsV2.ApiConnectorPlugin, question_1.ApiConnectionOptionItem.id]);
    }
